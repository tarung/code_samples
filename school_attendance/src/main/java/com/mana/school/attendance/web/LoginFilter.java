...
/**
 * Created by tarung on 8/5/2015 5:09 PM. This class is LoginFilter
 *
 * @author tarung
 * @Copyright
 */
...
//        httpServletResponse.sendRedirect("welcome.html"); // No logged-in user found, so redirect to login page.
        httpServletResponse.setHeader("Cache-Control", "no-cache, no-store, must-revalidate"); // HTTP 1.1.
        httpServletResponse.setHeader( "Pragma", "no-cache" );

        if (!( ( HttpServletRequest ) request ).getRequestURI( ).contains( "log" ) ) {
